...
export const HistoryPanel: React.FC<HistoryPanelProps> = ({ history, activeVersionId, onSelectVersion, onDeleteVersion, validatedVersionId, onImport, onPaste, onShare }) => {
  return (
    <div className="p-4 h-full flex flex-col">
      <div className="flex justify-between items-center mb-4 border-b border-white/10 pb-2">
        <h3 className="text-lg font-semibold text-white/80">Histórico de Versões</h3>
        <div className="flex space-x-2">
            <button 
              onClick={onPaste}
              className="flex items-center space-x-1 bg-white/10 hover:bg-white/20 text-white/80 py-1 px-2 rounded-lg transition text-xs"
      ...
      {(!history || history.length === 0) ? (
        <div className="flex-grow flex items-center justify-center text-white/40 text-center text-sm p-4">
                ...
                <div className="flex justify-between items-center">
                  <div className="flex items-center space-x-3 overflow-hidden">
                    ...
                    <div className="truncate">
                        <p className={`font-bold truncate ${activeVersionId === version.id ? 'text-emerald-400' : 'text-white/80'}`}>
                  ...
                  <div className="flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    {onShare && (
                      <button
                        ...
                        className="text-white/40 hover:text-emerald-400 flex-shrink-0"
                        title="Compartilhar versão"
                      >
